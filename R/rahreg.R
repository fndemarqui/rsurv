...
#' @aliases rahreg
#' @export
#' @description Function to generate a random sample of survival data from accelerated hazard models.
...
#' glimpse(simdata)
#'
rahreg <- function(u, formula, baseline, beta, dist = NULL, package = NULL, lwr = 0, upr = Inf, data, ...){
  call <- match.call()
  if(!is.null(dist)){
    baseline <- dist
  }
  ...
  if(baseline == "exp"){
    warning("The AH model with exponential baseline distribution is non-identifiable!")
  }

  # v <- u^exp(-lp)
  # time <- qsurv(v, baseline, ...)*exp(lp)
  time <- rEH(u, baseline = baseline, lp1 = lp, lp2 = 0, package = package, lwr = lwr, upr = upr, ...)
