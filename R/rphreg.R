...
#' glimpse(simdata)
#'
rphreg <- function(u, formula, baseline, beta, dist = NULL, package = NULL, lwr = 0, upr = Inf, data, ...){
  call <- match.call()
  if(!is.null(dist)){
    baseline <- dist
  }
  ...
  time <- rYP(u, baseline = baseline, lp_short = lp, lp_long = lp, package = package, lwr = lwr, upr = upr, ...)
