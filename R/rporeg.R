...
#' @aliases rporeg
#' @export
#' @description Function to generate a random sample of survival data from proportional odds models.
...
#' }
#'
rporeg <- function(u, formula, baseline, beta, dist = NULL, data, ...){
  if(!is.null(dist)){
    baseline <- dist
  }
  if(missing(data)){
  ...
  time <- rYP(u, baseline = baseline, lp_short = lp, lp_long = 0, ...)
  return(time)
}
